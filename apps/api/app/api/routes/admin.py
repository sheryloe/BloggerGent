...
def _ops_health_report_dir() -> Path:
    return settings.storage_common_analysis_dir / "ops-health"
...
def rebuild_cloudflare_channel_assets(
    ...
    try:
        return rebuild_cloudflare_assets(
            ...
            public_url_strategy=request_payload.public_url_strategy,
...
def bootstrap_cloudflare_channel_assets(
    ...
    try:
        return bootstrap_cloudflare_assets(
            ...
            verify_transport=request_payload.verify_transport,
        )
    except ProviderRuntimeError as exc:
        db.rollback()
        raise HTTPException(status_code=exc.status_code or 502, detail=exc.detail or exc.message) from exc
...
def backfill_blogger_editorial_labels(
    ...
    return {
        ...
        "task_id": task.id,
        "report_path": str(
            settings.storage_common_analysis_dir
            / "blogger"
            / f"blogger-editorial-label-backfill-{task.id}.json"
        ),
        "sync_results": [],
        "sheet_sync": None,
        "items": [],
