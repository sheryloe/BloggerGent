...
DEFAULT_SETTINGS: dict[str, DefaultSetting] = {
    ...
    "admin_auth_enabled": DefaultSetting("false", "관리자 인증 사용 여부"),
    "admin_auth_username": DefaultSetting("admin", "관리자 인증 사용자명"),
    "admin_auth_password": DefaultSetting("", "관리자 인증 비밀번호", True),
    ...
    "text_runtime_kind": DefaultSetting(settings.text_runtime_kind, "Primary text runtime kind for standard blog generation"),
    "text_runtime_model": DefaultSetting(settings.text_runtime_model, "Primary text runtime model for standard generation"),
    "image_runtime_kind": DefaultSetting(settings.image_runtime_kind, "Primary image runtime kind"),
    "codex_job_timeout_seconds": DefaultSetting(
        str(settings.codex_job_timeout_seconds),
        "Codex queue bridge timeout in seconds",
    ),
    "openai_usage_hard_cap_enabled": DefaultSetting(
        str(settings.openai_usage_hard_cap_enabled).lower(),
        "Deprecated compatibility flag. API-based stages still enforce the OpenAI hard cap in live mode.",
    ),
    ...
    "topic_discovery_provider": DefaultSetting(
        ...
        "Default topic discovery provider for standard generation. Refactor batches route separately.",
    ),
    ...
    "same_topic_cooldown_days": DefaultSetting("3", "Cooldown days for repeating similar topics regardless of angle"),
...
SETTING_DESCRIPTION_OVERRIDES_KO: dict[str, str] = {
    ...
    "admin_auth_password": "관리자 인증 비밀번호",
    ...
    "text_runtime_kind": "기본 텍스트 런타임",
    "text_runtime_model": "기본 텍스트 런타임 모델",
    "image_runtime_kind": "기본 이미지 런타임",
    "codex_job_timeout_seconds": "Codex 큐 대기 시간(초)",
    "openai_usage_hard_cap_enabled": "OpenAI 사용량 하드캡 사용",
    ...
    "same_topic_cooldown_days": "유사 주제 재사용 제한(일)",
...
def get_blogger_config(db: Session) -> dict:
    from app.services.platform.blog_service import list_blog_profiles, list_blogs
