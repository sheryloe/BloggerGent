...
from app.services.integrations.help_service import get_help_topic, list_help_topics, search_help_topics
...
def normalize_telegram_publish_url(url: str | None) -> str:
    ...
    if hostname:
        ...
        normalized_netloc = str(parsed.netloc or "").strip().lower()
    query_pairs = [
        (key, value)
        for key, value in parse.parse_qsl(parsed.query, keep_blank_values=False)
        if not str(key or "").strip().lower().startswith("utm_")
        and str(key or "").strip().lower() not in {"fbclid", "gclid", "igshid", "mc_cid", "mc_eid"}
    ]
    query_pairs.sort(key=lambda item: (str(item[0]).lower(), str(item[1])))
    normalized_path = parsed.path.rstrip("/") or "/"
    normalized_query = parse.urlencode(query_pairs, doseq=True)
    return parse.urlunsplit(("", normalized_netloc, normalized_path, normalized_query, "")).lstrip("//")
...
def _send_telegram_message(
    ...
    *,
    ...
    if dedupe_key:
        if (
            db.query(TelegramDeliveryEvent.id)
            .filter(TelegramDeliveryEvent.dedupe_key == dedupe_key)
            .filter(TelegramDeliveryEvent.status == "sent")
            .first()
        ):
    ...
    delivery_status = str(result.get("delivery_status") or "failed").strip().lower() or "failed"
    _log_delivery_event(
        db,
        chat_id=chat_id,
        message_type=message_type,
        status=delivery_status,
        dedupe_key=dedupe_key if delivery_status == "sent" else None,
        ...
        payload={
            ...
            "requested_dedupe_key": dedupe_key,
            **(extra_payload or {}),
        },
    )
    db.commit()
...
def send_telegram_post_notification(
    ...
    *,
    ...
    normalized_url = normalize_telegram_publish_url(post_url)
    dedupe_suffix = normalized_url or str(post_url or "").strip()
    return _send_telegram_message(
        db,
        bot_token=bot_token,
        chat_id=chat_id,
        text="\n".join(lines),
        message_type="publish_notification",
        dedupe_key=f"publish:{dedupe_suffix}",
        extra_payload={
            "raw_url": str(post_url or "").strip(),
