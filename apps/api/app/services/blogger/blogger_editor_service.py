...
def sync_blogger_post_search_description(
    db: Session,
    *,
    blogger_blog_id: str,
    blogger_post_id: str,
    description: str,
    article_id: int = 0,
) -> SearchDescriptionSyncResult:
    if not str(blogger_blog_id or "").strip():
        raise BloggerEditorAutomationError("This Blogger blog id is empty.", status_code=400)
    if not str(blogger_post_id or "").strip():
        raise BloggerEditorAutomationError("This Blogger post id is empty.", status_code=400)
    if not str(description or "").strip():
        raise BloggerEditorAutomationError("Search description is empty.", status_code=400)
    ...
    editor_url = _build_editor_url(str(blogger_blog_id), str(blogger_post_id), account_index)
    ...
    try:
        with sync_playwright() as playwright:
            ...
            try:
                ...
                _fill_description(page, str(description or "").strip())
                _save_editor(page)
            finally:
                page.close()
    ...
    return SearchDescriptionSyncResult(
        ...
        blogger_post_id=str(blogger_post_id),
...
def sync_article_search_description(db: Session, article: Article) -> SearchDescriptionSyncResult:
    ...
    return sync_blogger_post_search_description(
        db,
        blogger_blog_id=str(article.blog.blogger_blog_id or ""),
        blogger_post_id=str(article.blogger_post.blogger_post_id or ""),
        description=str(article.meta_description or "").strip(),
