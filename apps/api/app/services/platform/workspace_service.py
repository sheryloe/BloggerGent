...
from app.services.ops.metric_ingestion_service import run_workspace_metric_sync_schedule, sync_channel_metrics
from app.services.platform.platform_oauth_service import build_platform_authorization_url, refresh_platform_access_token
from app.services.platform.platform_publish_service import (
...
RUNTIME_LABELS: dict[str, str] = {
    ...
    "unexpected_openai_text": "Unexpected OpenAI Text",
    "other": "기타",
}
...
def _runtime_usage_bucket(event: AIUsageEvent) -> str:
    ...
    if "claude" in combined:
        return "claude_cli"
    if "openai_image" in combined or "/images" in combined or "gpt-image" in combined or "dall-e" in combined:
        return "openai_image"
    if "openai" in combined or "openai_text" in combined or "gpt-" in combined:
        return "unexpected_openai_text"
    ...
    if dialect_name == "postgresql":
        try:
            ...
            bucket = case(
                ...
                (combined.ilike("%openai_image%"), literal("openai_image")),
                (combined.ilike("%/images%"), literal("openai_image")),
                (combined.ilike("%gpt-image%"), literal("openai_image")),
                (combined.ilike("%dall-e%"), literal("openai_image")),
                (combined.ilike("%openai%"), literal("unexpected_openai_text")),
                (combined.ilike("%gpt-%"), literal("unexpected_openai_text")),
