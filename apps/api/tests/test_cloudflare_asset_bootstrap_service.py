...
def test_bootstrap_cloudflare_assets_can_verify_integration_proxy_transport(tmp_path: Path, monkeypatch) -> None:
    db = _create_db()
    try:
        local_root = tmp_path / "storage" / "images" / "Cloudflare"
        channel = ManagedChannel(
        ...
        monkeypatch.setattr(
            ...
            lambda *_args, **_kwargs: {
                ...
                "public_base_url": "https://api.dongriarchive.com",
            },
        )
        monkeypatch.setattr(
            bootstrap_service,
        ...
        monkeypatch.setattr(
            ...
            lambda *_args, **_kwargs: (
                "https://api.dongriarchive.com/assets/assets/media/cloudflare/dongri-archive/bootstrap/probe.webp",
                {
                    "object_key": "assets/media/cloudflare/dongri-archive/bootstrap/probe.webp",
        ...
        monkeypatch.setattr(
            bootstrap_service,
            "_verify_public_probe_url",
            lambda *_args, **_kwargs: (True, 200, "image/webp"),
        )
        deleted: list[str] = []
        monkeypatch.setattr(
            bootstrap_service,
            "delete_cloudflare_r2_asset",
            lambda *_args, **_kwargs: deleted.append(_kwargs["object_key"]),
        )
        ...
        result = bootstrap_service.bootstrap_cloudflare_assets(
            ...
            bucket_name="dongriarchive-cloudflare",
            verify_bucket=True,
            verify_transport="integration_proxy",
        )

        assert result["verify_transport"] == "integration_proxy"
        assert result["bucket_exists"] is True
        assert result["bucket_verified"] is False
        assert result["transport_verified"] is True
        assert result["public_base_url"] == "https://api.dongriarchive.com/assets"
        assert result["sample_uploaded_keys"] == ["assets/media/cloudflare/dongri-archive/bootstrap/probe.webp"]
        assert result["sample_public_urls"] == [
            "https://api.dongriarchive.com/assets/assets/media/cloudflare/dongri-archive/bootstrap/probe.webp"
        ]
        assert deleted == ["assets/media/cloudflare/dongri-archive/bootstrap/probe.webp"]
    finally:
        _close_db(db)
...
def test_ensure_cloudflare_r2_bucket_verifies_existing_bucket_with_existence_check(monkeypatch) -> None:
    db = _create_db()
    try:
        monkeypatch.setattr(
            storage_service,
        ...
        assert result["bucket_verified"] is True
        assert bucket_calls == ["HEAD https://acct-123.r2.cloudflarestorage.com/dongriarchive-cloudflare"]
        assert len(object_calls) == 3
    finally:
        _close_db(db)


def test_ensure_cloudflare_r2_bucket_raises_when_bucket_missing_without_create(monkeypatch) -> None:
    db = _create_db()
    try:
        monkeypatch.setattr(
            storage_service,
        ...
        monkeypatch.setattr(storage_service.httpx, "request", lambda *_args, **_kwargs: _FakeResponse(404, text="NoSuchBucket"))

        with pytest.raises(Exception, match="bucket does not exist"):
            storage_service.ensure_cloudflare_r2_bucket(
                db,
                bucket_name="dongriarchive-cloudflare",
                verify=False,
                create_if_missing=False,
