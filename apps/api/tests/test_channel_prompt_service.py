...
def _create_blog(db: Session, *, blog_id: int, slug: str, name: str) -> Blog:
    blog = Blog(
        id=blog_id,
        name=name,
        slug=slug,
...
def test_build_prompt_flow_syncs_blogger_backups(db: Session, tmp_path: Path) -> None:
    ...
    assert flow.steps[0].backup_relative_path == "channels/blogger/hidden-korea/steps/01-topic-discovery.md"
    assert flow.steps[0].backup_exists is True
    assert (tmp_path / "prompts" / "channels" / "blogger" / "hidden-korea" / "channel.json").exists()
    assert (
        tmp_path / "prompts" / "channels" / "blogger" / "hidden-korea" / "steps" / "02-article-generation.md"
...
def test_save_platform_prompt_step_persists_and_syncs_backup(db: Session, tmp_path: Path) -> None:
    ...
    assert initial_step.backup_relative_path == "channels/youtube/youtube-studio/steps/01-video-metadata-generation.md"
    ...
    assert (
        tmp_path / "prompts" / "channels" / "youtube" / "youtube-studio" / "steps" / "01-video-metadata-generation.md"
...
def test_build_prompt_flow_syncs_cloudflare_backups_to_channel_name_dir(
    ...
    assert flow.steps[0].backup_relative_path == (
        "channels/cloudflare/dongri-archive/steps/mystery/01-topic-discovery.md"
    )
    assert (
        tmp_path / "prompts" / "channels" / "cloudflare" / "dongri-archive" / "steps" / "mystery" / "01-topic-discovery.md"
