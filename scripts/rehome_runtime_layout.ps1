...
$defaults = @{
    ...
    storage = Resolve-PathFromEnvOrDefault -EnvMap $envMap -Key "API_STORAGE_DIR" -DefaultValue "./storage" -RepoRoot $repoRoot
}
...
$backupRoot = Join-Path $targetStorageRoot "backups"
$reportsRoot = Join-Path $targetStorageRoot "reports"
