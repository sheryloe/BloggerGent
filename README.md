...
## Search Console Playwright (Manual Per-Post Mode)

This repository now uses manual indexing for Search Console:

- No daily auto batch indexing.
- Indexing is executed per post from `/google`.
- Each row has two actions:
  - `색인 확인` (status refresh via URL Inspection API)
  - `색인 실행(1건)` (Playwright click automation in Search Console UI)

### Required Settings

Set these keys in Settings (or sync to `.env`):

```dotenv
BLOGGER_PLAYWRIGHT_ENABLED=true
BLOGGER_PLAYWRIGHT_CDP_URL=http://host.docker.internal:9223
AUTOMATION_GOOGLE_INDEXING_ENABLED=false
```

### PowerShell Runbook

1. Start browser for Search Console (CDP + persistent profile)
```powershell
cd D:\Donggri_Platform\BloggerGent
powershell -ExecutionPolicy Bypass -File .\scripts\start-search-console-browser.ps1
```

2. Sign in to Search Console in the opened browser and keep it open.

3. Optional API-level manual execution (single URL)
```powershell
curl -X POST "http://localhost:7002/api/v1/google/indexing/request-playwright" `
  -H "Content-Type: application/json" `
  -d "{\"count\":1,\"run_test\":false,\"urls\":[\"https://example.com/post\"],\"target_scope\":\"blogger\"}"
```

4. Optional status refresh (single URL)
```powershell
curl -X POST "http://localhost:7002/api/v1/google/indexing/status-refresh" `
  -H "Content-Type: application/json" `
  -d "{\"urls\":[\"https://example.com/post\"],\"target_scope\":\"blogger\",\"force\":true}"
```

### API Endpoints

- `POST /api/v1/google/indexing/request-playwright`
  - supports `urls` for per-post execution.
- `POST /api/v1/google/indexing/status-refresh`
  - refreshes index status for specified URLs.
